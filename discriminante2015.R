...
xbarca # Média das variáveis para a amostra 1
xbarpe # Média das variáveis para a amostrea 2

Sca <- cov(dados[dados$grupo == 1,1:2],dados[dados$grupo == 1,1:2])
Spe <- cov(dados[dados$grupo == 2,1:2],dados[dados$grupo == 2,1:2])

Sca # Matriz varcovar da amostra 1
Spe # Matriz varcovar da amostra 2


## TESTE DE BARTTLET ##

dados11 <- dados[dados$grupo == 1,1:2 ]
dados11 # valores das variáveis x1 e x2 na amostra 1
dados12 <- dados[dados$grupo == 2,1:2 ]
dados12 # valores das variáveis x1 e x2 na amostra 2

dim(dados11) # dimnesão da amostra 1 = 12 amostras
dim(dados12) # dimensão da amostrea 2 = 12 amostras
...
# Função disciminante de Fisher para a amostra
s1 <- Sca # matriz varcovar da amostra 1
s1
s2 <- Spe # matriz varcovar da amostra 2
s2
sp <- (((n1-1)*s1) + ((n2-1)*s2))/(n-2)
sp # Função disciminante de Fisher para a amostra
...
b
qui_cal <- -(1-a)*b
qui_cal # qui-calculado = 0,95
v <- (p*(p+1))/2
v
qui_tab <- qchisq(0.95, df=v)    ##  colocar o complementar do nível de significância ##
qui_tab # tabelado = 7.81

# qui-calculado < qui-tabelado ??????

# Discriminar cafe - pecuária
# método de Fisher, supondo SIGMAC = SIGMAV (varcovar igual pras duas amostras)
...
S <- (((n1-1)*Sca) + ((n2-1)*Spe))/(n-2) # matriz varcovar ponderada entre as duas amostras
S
...
Lpadro <- L * (1/(num))
Lpadro # var x2 exerce maior influência na classificação 

# valor da função discriminante para cada indivíduo
pre <- L %*% t(dados[,1:2]) 
pre <- as.vector(pre)
pre
