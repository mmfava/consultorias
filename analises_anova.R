# Script R para Análise de Correlação e GLM
# Marília Melo Favalesso - mariliabioufpr@gmail.com

#### CHAMAR OS DADOS ####
dados <- read.csv2(file.choose(), header = TRUE, dec = ",") # Dados para correlação
attach(dados)
list(dados)
names(dados)
library(ggplot2)
...
model1 <- ggplot(dados, aes(x = rur, y = abund)) +
  ...
  annotate("text", x = 25, y = 30, label = "rho=0.10") # Correlação de Spearman usa-se rho.
model1  

# Resultado final não significativo. Correlação fraca entre abundância de macroinvertebrados e área rural.
...
## Carregar os dados ##
abundancia <- read.csv2(file.choose(), header = TRUE, dec = ",") # Abundância riachos
abiotico <- read.csv2(file.choose(), header = TRUE, dec = ",") # CCA-abióticos
...
text(6, 7, "Marília Melo Favalesso")
text(c(2, 3), c(8, 6), c("Marília", "Favalesso"))
